...
for file in html_files:
    ...
    # If it has a <header> wrapping the nav, strip from <!-- TopNavBar --> to </header>
    # Note: Some files might not have <!-- TopNavBar -->, let's use <nav if needed
    
    # We will use regex to find the entire block that represents the top nav
    # Usually it's right after <body> or <div id="app"> etc.
    # Let's find <header class="fixed top-0 ...">...</header>
    
    pattern_header = r'<header class="fixed top-0.*?</header>'
    pattern_nav = r'<nav class="fixed top-0.*?</nav>'
    
    if re.search(pattern_header, content, re.DOTALL):
        content = re.sub(pattern_header, master_nav, content, flags=re.DOTALL, count=1)
    elif re.search(pattern_nav, content, re.DOTALL):
        content = re.sub(pattern_nav, master_nav, content, flags=re.DOTALL, count=1)
...
print("Cleaned up headers/navs and applied master nav")
